...
export default function Signup() {
  ...
  async function handleSignup(e) {
    ...
    setLoading(true)
    try {
      await signInWithOtp(email, `${window.location.origin}/app`, { full_name: name })
      setSent(true)
    } catch(err) {
          ...
          {sent ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: '#8A8880', fontSize: 14, lineHeight: 1.6 }}>
                We sent a magic link to <strong style={{ color: '#F0EDE6' }}>{email}</strong>.<br />Click it to activate your free account.
              </p>
            </div>
          ) : (
            <>
              <div style={{ display: 'inline-block', background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.25)', borderRadius: 20, padding: '4px 14px', fontSize: 11, fontWeight: 600, color: '#C9A84C', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 16 }}>Free — no credit card</div>
